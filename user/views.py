...
from weather.models import city, hourly_forecast_log, daily_forecast_log, information  # country
...
def signUP(request):
    ...
    if request.method == 'POST':
        ...
        data.city = city_name
        data.email = email
        # c est un ficher text nommé ccdata dans le repertoire de projet
        ...
        if User.objects.filter(email=email).exists():
            ...
            cityid = 100001  # Default Rabat
            userCity.objects.create(username=username, city_id=cityid)
            messages.info(request, "City not available. Your account created with Default city as Rabat, Morocco")
            return redirect('/')
        else:
            # create_user
...
def searchbar(request):
    ...
    try:
        ...
        qcountry = cityInfo.country_id.country_name
        info.msg = ""

    except:
        info.msg = "Please search other locations..."
        cityInfo = city.objects.get(city_name__iexact="Rabat")
        cityid = cityInfo.id
        url = "http://api.openweathermap.org/data/2.5/weather?lat={}&lon={}&appid={}&units=metric".format(
            cityInfo.latitude,
            cityInfo.longitude,
