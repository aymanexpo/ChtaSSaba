...
def hourly_forecast(request):
    ...
    while i != totalcity:
        ...
        url = "http://api.openweathermap.org/data/2.5/onecall?lat={}&lon={}&appid={}&units=metric".format(x.latitude,
                                                                                                          x.longitude,
                                                                                                          "{add your api code}")
        try:
            data = requests.get(url).json()
    ...
    return render(request, "weather/admin.html", {'what': 'hourly_forecast'})
...
def daily_forecast(request):
    ...
    while i!=totalcity:
        ...
        url = "https://api.weatherbit.io/v2.0/history/daily?lat={}&lon={}&start_date={}&end_date={}&key={}".format(
            ...
            str(start), str(end), "{add your api code}")
...
def daily_forecast_delete(request):
    ...
    return render(request, "weather/admin.html", {'what': 'daily_forecast_deleted'})
